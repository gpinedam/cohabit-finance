...
import { deleteWishlistItem, listWishlistItems } from '../services/api'
...
function WishlistCard({ item, onEdit, onDelete }) {
  const [confirmDelete, setConfirmDelete] = useState(false)

  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex gap-3 group">
      {/* Thumbnail */}
...
export default function Wishlist() {
  ...
  const [filterStars, setFilterStars] = useState(null)  // null = all
  ...
  }, [filterStars, minPrice, maxPrice, sort])
    ...
    <div className="pt-16 pb-28 max-w-lg mx-auto">
      ...
      <div className="px-5 pt-5 pb-3 flex items-end justify-between">
        <div>
          ...
          <h1 className="text-xl font-bold text-slate-900 leading-tight mt-0.5">Deseos</h1>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-base font-bold text-slate-800 mb-1">
              {hasActiveFilters ? 'Sin resultados' : 'Tu lista está vacía'}
            </p>
            <p className="text-sm text-slate-400 mb-6 max-w-[220px] leading-relaxed">
              {hasActiveFilters
                ? 'Prueba con otros filtros'
                : 'Añade cosas que quieras comprar, con tu propia valoración'}
            </p>
            {!hasActiveFilters && (
              <button
                onClick={openCreate}
